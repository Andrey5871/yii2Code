          ...
          <form class="auth">
            ...
            <div class="form-group">
              ...
              <input type="password" class="form-control" name="password" id="manufacturPassword" placeholder="Введите пароль" required>
						...
						<a href="#" class="pswForgoutHref">Забыли пароль?</a><br>
             <button class="btn btn-grey mt-2 pr-4 pl-4 pt-2 pb-2 login" type="button">Войти</button>
          </form>

          </div>
          </div>
					<div class="row pswForgout" style="display:none">
						<hr style="width:100%">
							<div class="col-12">

								<h5 class="text-center pb-3">Для восстановления пароля введите ваш email</h5>
								<div class="action">
										<!--BEGINACTIONEMAILFOF-->
										<?php if (isset($action)): ?>
												<?php echo $action; ?>
								...
								<form>
									<div class="form-group">
										<!-- <label for="exampleInputEmail1">Введите email</label> -->
										<input type="email" class="form-control" name="emailForgout" id="emailForgout"  placeholder="Введите email" required>
									</div>
									<button class="btn btn-grey mt-2 pr-4 pl-4 pt-2 pb-2" type="button">Отправить</button>
								</form>
							</div>
					</div>
          </div>
									...
									var textInput = $(el).attr('placeholder');
										...
										actionError.push("email");
											...
											$.ajax({
															...
															type: 'POST',
															data: {"_csrf-frontend": '<?=Yii::$app->request->getCsrfToken()?>', password:password, email:email},
															beforeSend: function(){
														...
														.done(function(data) {
															 var beginCityResult = data.indexOf("<!-- hrefredirectBegin -->");
																...
																if (result.trim() == "Пользователь не найден" || result.trim() == "Неверный email или пароль") {
